...
import os, webbrowser, threading, signal
...
def read_db_bit(plc, db_number, byte_index, bit_index):
    data = plc.read_area(Area.DB, db_number, byte_index, bit_index)
    bitvalue = get_bool(data, 0, bit_index)
    print(f"DB{db_number}.DBX{byte_index}.{bit_index} value: ", bitvalue)
    return bitvalue

def write_db_bit(plc, db_number, byte_index, bit_index, value):
    data = plc.read_area(Area.DB, db_number, byte_index, bit_index)
    set_bool(data, 0, bit_index, value)
    plc.write_area(Area.DB, db_number, byte_index, data)
    print(f"DB{db_number}.DBX{byte_index}.{bit_index} value set to: ", value)
    return str(value)
...
<body>
    ...
    <div class="container">
        <h1>Read and Write PLC DBs from Website with Python</h1>
        <div class="form-section">
            <form method="POST" action="/trigger">
                <div class="form-grid">
                    <div class="half">
                        <label for="dbnumber">DB Number </label><input id="db_number" name="db_number" value=1 required>
                        <label for="db_byteindex">DB Byte Index </label><input id="db_byteindex" name="db_byteindex" value=1 required>
                        <label for="db_bitindex">DB Bit Index </label><input id="db_bitindex" name="db_bitindex" value=1 required>
                        <label for="db_value">DB Bit Value </label><input id="db_value" name="db_value" value=1 required>
        ...
        <div class="form-section">
            <form id="getstatus" method="POST" action="/get-status">
                <div class="form-grid">
                    <div class="half">
                        <p><b>DB{data_address["db_number"]}.DBX{data_address["db_byteindex"]}.{data_address["db_bitindex"]}</b> Status: </p><h1 id="db_bit_status">{statusColor}</h1>
...
def trigger():
    data_address["db_number"] = request.form.get("db_number")
    ...
    try:
        write_db_bit(plc=client, db_number=int(data_address["db_number"] ), byte_index=int(data_address["db_byteindex"] ), bit_index=int(data_address["db_bitindex"]), value=int(db_value))
    except Exception as e:
...
def get_log():
    ...
    try:
        ...
        status = read_db_bit(plc=client, db_number=int(data_address["db_number"]), byte_index=int(data_address["db_byteindex"]), bit_index=int(data_address["db_bitindex"]))
        if status == 1:
            color = "ON"
        else:
            color = "OFF"
