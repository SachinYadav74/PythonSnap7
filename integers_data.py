...
data_address = {'db_number':1, 'db_byteindex':14, }
...
def write_int_db(plc, db_number, byteindex, value):
    """Write integer value to a PLC DB block."""
    data = bytearray(2)  # Integer takes 2 bytes
    set_int(data, 0, value)
    plc.db_write(db_number, byteindex, data)
...
<body>
    ...
    <div class="container">
        <h1> <b>Read</b> and <b>Write</b> PLC Integers in DBs from Website with Python</h1>
        <div class="form-section">
            <form method="POST" action="/trigger">
                <div class="form-grid">
                    <div class="half">
                        <label for="dbnumber">DB Number </label><input id="db_number" name="db_number" value=1 required>
                        <label for="db_byteindex">DB Byte Index </label><input id="db_byteindex" name="db_byteindex" value=14 required>
        ...
        <div class="form-section">
            <form id="getstatus" method="POST" action="/get-status">
                <div class="form-grid">
                    <div class="half">
                        <p><b>DB{data_address["db_number"]}.DBW{data_address["db_byteindex"]}</b> Status: </p><h1>{statusColor}</h1>
...
def trigger():
    ...
    try:
        write_int_db(plc=client, db_number=int(data_address["db_number"] ), byteindex=int(data_address["db_byteindex"] ), value=int(db_value))
    except Exception as e:
...
def get_log():
    try:
        status = read_int_FromPLC(plc=client, db_number=int(data_address["db_number"]), byteindex=int(data_address["db_byteindex"]), size=2)
        ...
        return render_page(statusColor="0")
